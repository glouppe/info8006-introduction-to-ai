...
\begin{itemize}
    ...
    \item Deadline : 30th of November ;
    \item Deliverable 1 : A python file with a class named "Solver" which can be initialised with \emph{Solver(X,k)} and with the following method : $solve(M)$, where $M$ is a NumPy matrix of integers, $X$ your symbol (either 1 or 2), which return a tuple (A,B) with A and B strictly positive natural numbers ;
    \begin{itemize}
    	\item \danger If your tuple violates the game constraints, you simply miss the turn ;
        \item \danger Execution time of $solve(M)$ is limited !
    \end{itemize}
    \item Deliverable 2 : A report of max 3 pages which describes your work. Appendix with some statistics are more than welcome;
    \item Upload your deliverables (pattern : \emph{name}\_tictactoe.\{ pdf|py \}) at the \href{http://submit.montefiore.ulg.ac.be/student/viewprojects/INFO8006}{Montefiore submission platform}
