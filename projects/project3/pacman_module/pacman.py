...
#        GameState.explored.add(self)
    ...
    def initialize(self, layout, numGhostAgents=1000, hiddenGhosts=False, edibleGhosts=False,beliefStateAgent=None):
        """
        Creates an initial game state from a layout array (see layout.py).
        """
        self.data.initialize(layout, numGhostAgents, isGhostVisible=not hiddenGhosts, edibleGhosts=edibleGhosts, beliefStateAgent=beliefStateAgent)
...
class ClassicGameRules:
    ...
    def newGame(
            ...
            display,
            quiet=False,
            catchExceptions=False, hiddenGhosts=False, edibleGhosts=False):
        
        agents = [pacmanAgent] + ghostAgents[:layout.getNumGhosts()] + ([beliefStateAgent] if beliefStateAgent is not None else []) 
        initState = GameState()
        initState.initialize(layout, len(ghostAgents), hiddenGhosts=hiddenGhosts, edibleGhosts=edibleGhosts, beliefStateAgent=beliefStateAgent)
...
def runGame(
        ...
        beliefstateagent,
        displayGraphics,
        expout=np.inf,hiddenGhosts=False, edibleGhosts=False):
    ...
    game = rules.newGame(lay, pacman, ghosts, beliefstateagent, display, False, False, hiddenGhosts=hiddenGhosts, edibleGhosts=edibleGhosts)
    return game.run()
