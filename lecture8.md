...
Assume a **training set** of $N$ example input-output pairs
$$(\mathbf{x}\_1, y\_1), (\mathbf{x}\_2, y\_2), ..., (\mathbf{x}\_N, y\_N),$$
where
- $\mathbf{x}\_i \in \mathbb{R}^d$ are real-valued vectors describing the input data;
...
- To measure the accuracy of a hypothesis, we evaluate its predictions on a *test set* that is independent of the training set.
- A hypothesis $h$ **generalizes** well if it correctly predicts the value of $y$ for novel examples
    - i.e., if $P(Y=y|x,h)$ is high for novel examples $(x,y)$.
...
- Supervised learning can be done by choosing the hypothesis $h^\*$ that is most probable **on new data** $\mathbf{d}$:
$$h^* = \arg \max\_{h \in \mathcal{H}} P(h|\mathbf{d})$$
- By Bayes's rule, this is equivalent to
$$h^* = \arg \max\_{h \in \mathcal{H}} P(\mathbf{d}|h)P(h)$$
where $P(\mathbf{d}|h)$ = $\prod\_{(\mathbf{x}, y) \in \mathbf{d}} P(y|x,h)$
- This is the same as:
    - Maximum a posteriori estimation.
    - Maximum likelihood estimation if $P(h)$ is uniform.
- **Issue**: in practice, we often only have *finite* test set!
