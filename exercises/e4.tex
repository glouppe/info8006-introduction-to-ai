...
\begin{enumerate}
    ...
    \begin{solution}
        This is true iff $B \perp C | A$. This time, $(B, A, C)$ is not active because $A$ is known. $(B, D, C)$ is not active either because $D$ is unknown ($\vee$-structure). In $(B, F, A, C)$, the triplet $(B, F, A)$ is not active ($\vee$-structure).  $(F, A, C)$ is not active either. We can guarantee that $B \perp C | A$.
    \end{solution}
