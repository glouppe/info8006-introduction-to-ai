...
Modification
------------
- Adding colors to Pacman's path (l280)
...
class PacmanGraphics:
    ...
    def update(self, newState, show):
        ...
        #------------------
        if show == 1:
            # Retreive the current position of Pacman
            current_pos = agentState.getPosition()

            # Retreive the index
            c_index = [current_pos[0], current_pos[1]]

            # Add its position to its total path
            if c_index not in Pacman_path:
                Pacman_path.insert(0, c_index)

            # Update the path if Pacman went back on its trail
            else:
                Pacman_path.remove(c_index)
                Pacman_path.insert(0, c_index)

            # Color its path
            self.drawExpandedCells(Pacman_path)
