...
The distances come from `maze.py`, which floods the maze from a position and
caches the resulting map, so that the heuristic is the true remaining cost.
"""
...
def heuristic(state):
    ...
    distances = [
        maze_retrieve(state, pacman_position, (i, j), maze_map)
